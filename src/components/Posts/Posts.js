...
function Posts(props) {
  ...
  return (
    ...
    <section className='posts'>
        <button className='posts__button-new' onClick={props.openPopup}>Создать новый пост</button>
        <h2 className='posts__title'>Последние посты</h2>
        <div className='posts__grid-container'>
        {postsList}
        </div>
        <button className='posts__button-more' onClick={() => downloadPosts()}>Ещё</button>
    </section>
  )
}
